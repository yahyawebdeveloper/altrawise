...
/**
 * @author - Syed Anees
 * @description - Get the upload directory path passed from the upload form with 'upload_dir' name
 * so  REQUEST['upload_dir'] will give you directory path to upload file. This will be useful 
 * to store files with modules, sub modules and employee directory
 */
...
function get_upload_directory() {
    ...
    if (isset($_POST["upload_path"]) && $_POST["upload_path"] != null) {
        return $_POST["upload_path"];
    }

    //return the default files folder
    return 'files';
}
...
// for delete file requests
    ...
    $result = $uploader->getFilesFromServices(urldecode($_GET['param']));
    echo json_encode($result);
}
else {
